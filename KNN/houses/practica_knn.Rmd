...
values_x2 <- seq(1, 100, by = 1)
values_x3 <- seq(1, 100, by = 1)

matrix_expand <- expand.grid(values_x2, values_x3)

names(matrix_expand) <- c("X2", "X3")

price_prediction_matrix <- predict(knn_model_x2_x3, newdata = matrix_expand)

matrix_expand$Y <- price_prediction_matrix

z_matrix <- matrix(data = matrix_expand$Y, nrow = length(values_x2), ncol = length(values_x3))

plot_ly(z = ~z_matrix) %>% add_surface()
```
